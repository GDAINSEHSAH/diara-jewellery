...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    if (!visible) return;
    const tooltipTimer = setTimeout(() => setTooltip(true), 4000);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex items-center gap-3">
      {/* Tooltip */}
      <div
        className={`bg-white rounded-lg shadow-lg px-4 py-2.5 text-sm text-stone-700 transition-all duration-300 max-w-[200px] ${
          tooltip ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4 pointer-events-none"
        }`}
      >
        <p className="font-medium">Need help?</p>
        <p className="text-stone-500 text-xs mt-0.5">Chat with us on WhatsApp</p>
        {/* Tooltip arrow */}
      ...
      {/* WhatsApp button */}
      <a
        href="https://wa.me/919876543210?text=Hi%20DIARA!%20I%27d%20like%20to%20know%20more%20about%20your%20jewellery%20collection."
        target="_blank"
